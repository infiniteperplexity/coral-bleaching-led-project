...
// i just double checked the inverse path and it seems right
...
void setup() {
  ...
  for (int i=0; i<SIDE; i++)
  {
    ...
    for (int j=0; j<SIDE; j++)
    {
      ...
      square8[i][j] = square3[j][i];
      square9[i][j] = square3[17-j][17-i];
      triangle10[i][j] = triangle4[i][j];
      triangle11[i][j] = triangle4[17-i][j]; 
      square12[i][j] = square3[j][i];
...
//*************this is the code that implements masking ***********************
  ...
  String msg = "";
   //for this kind of thing, we need to nest x loops within y loops to make the display imtuitively match cartesian coordinates
...
void loop() {
  ...
  {
    //linear_test(strip.Color(0,0,0,255));
    //grid_test(strip.Color(0,0,0,255));
    grid_test(strip.Color(0,255,255));
  }
  else
  {
    //rainbow(20);
...
// **** Simplest testing, directly addresses the pixels to light them up ***************
...
// ***** This pattern lights up the pixels using the grid mapping, and it's currently being tested ********
static bool firstLoop = true;
void grid_test(int color) {
  int c = color;
  int pixel;
  //String msg = "";
  //String nxt;
  // **** I think we can just let it try to light up every panel even if they're not connected ******
  for (int i=0; i<(4*SIDE); i++)
  {
    for (int j=0; j<(4*SIDE); j++)
    {
      ...
      int i1 = i - i0*SIDE;
      int j1 = j - j0*SIDE;
      //c = strip.Color(255-3*j,0,0,255-3*i);
      pixel = panels[layout[i0][j0]][i1][j1] - 1;
      //nxt = pixel;
      //nxt += " ";
      //msg += nxt; 
      //msg = msg + String(" ") + String(pixel);
//      if (i==42 && j==15)
//      {
        //Serial.println(i0);
        //Serial.println(j0);
...
//      }
  ...
  strip.setPixelColor(920,strip.Color(0,255,0));
  firstLoop = false;
  strip.show();
}
